...
namespace CleanArchitecture.Application.Services
{
    public class BlogService : IBlogService
    {
        ...
        public async Task<bool> Add(BlogViewModel blog)
        {
            ...
            int id = await _blogRepository.Add(new Domain.Models.Blog { Article = blog.Article, CreatedAt = DateTime.Now, Title = blog.Title, UserId = blog.UserId, Categories = _mapper.Map<List<Category>>(blog.Categories)});
            return id > 0;
        }
        ...
        public List<BlogViewModel> GetAllByCategory(int categoryId)
        {
            ...
            foreach (var item in currentBlogs)
            {
                ...
                //}
                blogs.Add(new BlogViewModel { Article = item.Article, Title = item.Title, Categories = _mapper.Map<List<CategoryViewModel>>(item.Categories) });
            }
            return blogs;
        }
        ...
        public List<BlogViewModel> GetAllByTag(int tagId)
        {
            ...
            foreach (var item in currentBlogs)
            {
                ...
                blogs.Add(new BlogViewModel
                    {Article = item.Article, Title = item.Title, Tags = _mapper.Map<List<TagViewModel>>(item.Tags)});
            }
